...
- **`examples/` renamed to `implementation/`** — reference implementation lives in `implementation/reference/` (own `platformio.ini` with `esp32c3` + `esp8266` envs), consuming the library from `../..` exactly like an external project
...
- Pin and dummy-mode configuration moved to the implementation (`implementation/reference/src/config.h`); the `dummyMode` global was removed from `CroasterConstants.h`
